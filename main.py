...
class RootWidget(FloatLayout):
    ...
    def fileExistsPopup(self):
        ...
        self.downloadin.text="Local file"
    ...
    def ytDownloader(self, btn):
        ...
        try:
            ...
            return -1
        if (self.infos['codec'] == ''):
            self.infos['codec'] = str(yt.streams.all()[0]).split()[2].split("/")[1][:-1] #on chope l'extention
            self.infos['resolution'] = str(yt.streams.all()[0]).split()[3].split("\"")[1][:-1]
        try:
        ...
        self.infos['file'] = os.path.join(self.infos['path'], str(yt.__dict__).split("<title>")[1].split("</title>")[0].split(" - YouTube")[0] + "." + self.infos['codec'])
        if (os.path.isfile(self.infos['file'])):
            self.fileExistsPopup()
            return -1
        self.downloader.text = 'Downloading ...'
        # video = yt.get(self.infos['codec'], self.infos['resolution']) #on selectionne "cette" video TODO : PAS BON
        # self.start_down_thread(video)
        self.start_down_thread(yt)

    # def start_down_thread(self, video):
    def start_down_thread(self, yt):
        downar = threading.Thread(target=self.down_thread, args=(yt,))
        downar.start()
        calulator = threading.Thread(target=self.calc_thread, args=())
        calulator.start()
        self.pageTwo()

    def down_thread(self, yt):
        try:
            yt.streams.filter(file_extension=self.infos['codec']).first().download(self.infos['path'])
        except Exception as e:
            self.errorPopup(e)
            return -1
        self.infos['downloaded'] = True
    ...
    def create_progress_bar(self, progr, poz, sise, color=Color(.0, .0, .0, .5), vert=None):
        if not vert:
            vert = False   
        ...
        if (self.infos['arbo']):
            ...
            if(not os.path.exists(self.infos['where'])):
                try:
                    ...
                    return -1
        try:
#error android alors que les commandes marchent avec subprocess!
            
            self.set_opt_temp_files()
            song = AudioSegment.from_file(self.infos['file'],self.infos['codec'])
        except Exception as e:
            # self.errorPopup("ici")
            self.errorPopup(e)
            return -1
        if(self.titles.text.count("\n") < 1):
            ...
            times = "0:0"
            self.infos['titles'].append(re.sub(r"[ ]*[0-9]?:?[0-9]{1,2}:[0-9]{2}[ ]*", "", line))
            portee = times.count(":")
            tempSec = 0
            ...
            for i in range(self.titles.text.count("\n")+1):
                ...
                times = (re.findall(r"[0-9]?:?[0-9]{1,2}[:][0-9]{2}", line)[0])
                self.infos['titles'].append(re.sub(r"[ ]*[0-9]?:?[0-9]{1,2}:[0-9]{2}[ ]*", "", line))
                portee = times.count(":")
                tempSec = 0
                for j in range(0, portee + 1):
...
#error dans le coin sur le pc 
    ...
    def get_perm_files(self):
        self.perm_files = []
        print(tempfile.gettempdir())
        for fil in os.listdir(tempfile.gettempdir()):
            self.perm_files.append(fil)
            print(fil)

    def set_opt_temp_files(self):
        for fil in os.listdir(tempfile.gettempdir()):
            if fil not in self.perm_files:
                os.chmod(os.path.join(tempfile.gettempdir(),fil), '0777')
...
#TODO choisir format de sortie
    ...
    try:
        ...
        t.close
        os.remove(os.path.join(tempfile.gettempdir(),"test.txt"))
        l.debug("Tempdir writable")
    except Exception as e:
        l.debug(str(e))
    try:
        subprocess.check_output(["ls"])
    except Exception as e:
        l.debug("Sub shell commands error")
        l.debug(str(e))
    # error : TODO changer permission tempdir en python (faire un subprocess.call ne peut pas marcher car le process cree ne posede pas l'uuid de l'app)
    try:
        os.chmod(tempfile.gettempdir(), '0777')
    except Exception as e:
        l.debug("Tempdir not chmodable")
        l.warn(str(e))
    l.debug(platform)
    if platform == 'android':
        os.environ["TMP"] = "/sdcard/"
    l.debug(tempfile.gettempdir())
